...
export const Calendar = () => {
  const { clients, sessions, updateSession, addSession } = useStoreCtx()
  const now = new Date()
  ...
  return (
    <div className="px-4 pt-6 pb-28 max-w-lg mx-auto">
      ...
      <div className="flex items-center justify-between mb-5">
        <h1 className="text-2xl font-bold text-slate-900">{MONTHS[month]} {year}</h1>
        ...
        {weeks.map((week, wi) => (
          <div key={wi} className="grid grid-cols-7 gap-1">
            {week.map((day, di) => {
              ...
              const hasSessions = daySessions.length > 0 && isCurrentMonth
              ...
              return (
                <button
                  ...
                  } ${isSelected ? 'bg-[#635BFF] text-white' : isToday ? 'bg-indigo-50 text-[#635BFF]' : 'hover:bg-slate-100'}`}
                >
                  <span className={`text-sm font-medium ${isSelected ? 'text-white' : isToday ? 'text-[#635BFF]' : 'text-slate-700'}`}>
                    {day.getDate()}
                  </span>
                  {hasSessions && (
      ...
      {selectedDate && (
        <div className="mt-5">
          <div className="flex items-center justify-between mb-3">
            ...
            <button onClick={() => setAddSessionModal(true)} className="text-sm text-[#635BFF] font-medium">+ Add</button>
          </div>
          {selectedSessions.length === 0 ? (
            <Card><p className="text-sm text-slate-400 text-center py-3">No sessions. Tap + Add to create one.</p></Card>
          ) : (
      ...
      {/* Add session modal */}
      <Modal open={addSessionModal} onClose={() => setAddSessionModal(false)} title={`Add Session — ${selectedDate ? formatDisplay(selectedDate) : ''}`}>
